...
- **Backend (`apps/gengar/`)**: NestJS 10 + TypeORM + PostgreSQL (pgvector). Deployed to Heroku.
- **Frontend (`apps/gengar_fe/`)**: Next.js 14 (App Router) + TypeScript + Tailwind CSS. Deployed to Vercel.
...
|---|---|---|
| `backend-ci.yml` | PR/push to `main` touching `apps/gengar/**` | Lint, build, test |
| `frontend-ci.yml` | PR/push to `main` touching `apps/gengar_fe/**` | Lint, build |
| `deploy-backend.yml` | Push to `main` touching `apps/gengar/**` | Deploy to Heroku + run migrations |
...
- Frontend communicates via `gengarApi` client (`apps/gengar_fe/src/services/api.ts`) using Axios
...
Frontend (`.env` in `apps/gengar_fe/`):
...
2. Build the backend to verify (`cd apps/gengar && yarn build`)
3. Update the frontend API client (`apps/gengar_fe/src/services/api.ts`)
4. Add React Query hooks if needed (`apps/gengar_fe/src/hooks/`)
