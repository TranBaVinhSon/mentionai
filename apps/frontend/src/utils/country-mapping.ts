...
const COUNTRY_DATA: Record<string, CountryInfo> = {
  ...
  'AQ': { code: 'AQ', name: 'Antarctica', flag: '🇦🇶' },
  ...
  'GS': { code: 'GS', name: 'South Georgia and the South Sandwich Islands', flag: '🇬🇸' },
  ...
  'HM': { code: 'HM', name: 'Heard Island and McDonald Islands', flag: '🇭🇲' },
  ...
  'IO': { code: 'IO', name: 'British Indian Ocean Territory', flag: '🇮🇴' },
  ...
  'PM': { code: 'PM', name: 'Saint Pierre and Miquelon', flag: '🇵🇲' },
  ...
  'SH': { code: 'SH', name: 'Saint Helena', flag: '🇸🇭' },
  'SI': { code: 'SI', name: 'Slovenia', flag: '🇸🇮' },
  'SJ': { code: 'SJ', name: 'Svalbard and Jan Mayen', flag: '🇸🇯' },
  ...
  'UM': { code: 'UM', name: 'United States Minor Outlying Islands', flag: '🇺🇲' },
...
export function getCountryInfo(input: string): CountryInfo | null {
  ...
  if (COUNTRY_DATA[inputUpper]) {
    return COUNTRY_DATA[inputUpper];
  }
  
  const code = COUNTRY_NAME_TO_CODE[inputLower];
...
export function formatCountryDisplay(
  input: string, 
  type?: 'flag' | 'name' | 'full',
  options: CountryDisplayOptions = { showFlag: true, showCode: false }
): string {
